...
class EditContactViewModel @Inject constructor(
    ...
    sealed class MainViewAction {
        sealed class Navigate : MainViewAction() {
            data class ContactVip(val currentId: Int) : Navigate()
        }
    }
    ...
    val mainViewAction: LiveData<Event<MainViewAction>> = liveData() {
        getCurrentContactIdChannelUseCase.invoke().collect { id ->
            id?.let {
                emit(Event(MainViewAction.Navigate.ContactVip(it)))
            }
        }
    }
    ...
    val singleContactViewStateLiveData = liveData {
        combine(singleContactViewStateMutableSharedFlow, test, test2) { singleViewState, aggregatedPhotos, isSold ->
            Log.i("GetId", "singleViewState edit : ${singleViewState}")
    ...
    val editContactViewStateLiveData = liveData {
        combine(editContactViewStateMutableSharedFlow, test, test2) { editContactViewState, aggregatedPhotos, isSold ->
            Log.i("GetId", "singleViewState edit : ${editContactViewState}")
    ...
    fun onVipSettingsClicked(id: Int) {
        setCurrentContactIdUseCase.invoke(id)
    }
